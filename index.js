  ...
  return {
    ...
    resolver: async (source, args, context, info) => {
      try {
        ...
        if (!input.length) throw new GraphQLError3("No values were provided!");
  ...
  return {
    ...
    resolver: async (source, args, context, info) => {
      try {
        ...
        if (!Object.keys(input).length) throw new GraphQLError3("Unable to update with no values specified!");
...
var generateInsertArray2 = (db, tableName, table, baseType) => {
  ...
  return {
    ...
    resolver: async (source, args, context, info) => {
      try {
        const input = remapFromGraphQLArrayInput(args.values, table);
        if (!input.length) throw new GraphQLError4("No values were provided!");
        const parsedInfo = parseResolveInfo2(info, {
          deep: true
        });
  ...
  return {
    ...
    resolver: async (source, args, context, info) => {
      try {
        ...
        const input = remapFromGraphQLSingleInput(set, table);
        if (!Object.keys(input).length) throw new GraphQLError4("Unable to update with no values specified!");
        let query = db.update(table).set(input);
        if (where) {
          const filters = extractFilters(table, tableName, where);
          query = query.where(filters);
...
var generateInsertArray3 = (db, tableName, table, baseType) => {
  ...
  return {
    ...
    resolver: async (source, args, context, info) => {
      try {
        const input = remapFromGraphQLArrayInput(args.values, table);
        if (!input.length) throw new GraphQLError5("No values were provided!");
        const parsedInfo = parseResolveInfo3(info, {
          deep: true
        });
        const columns = extractSelectedColumnsFromTreeSQLFormat(
  ...
  return {
    ...
    resolver: async (source, args, context, info) => {
      try {
        ...
        const input = remapFromGraphQLSingleInput(set, table);
        if (!Object.keys(input).length) throw new GraphQLError5("Unable to update with no values specified!");
        let query = db.update(table).set(input);
        if (where) {
          const filters = extractFilters(table, tableName, where);
          query = query.where(filters);
